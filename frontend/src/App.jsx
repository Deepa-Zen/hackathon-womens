...
import { 
  ...
  Footprints, CalendarDays, MapPin, Globe, Image, Camera, Paperclip, Video, Map, Layout, Locate, Award, Languages, ChevronDown, Menu, LogOut, Settings2, HelpCircle, Volume2, Play
} from 'lucide-react';
...
export default function App() {
  ...
  const [userWeight, setUserWeight] = useState(() => localStorage.getItem('mhc_weight') || '60');
  const [userHeight, setUserHeight] = useState(() => localStorage.getItem('mhc_height') || '160');
  const [state, setState] = useState('Tamil Nadu');
  const [showLangMenu, setShowLangMenu] = useState(false);
  const [activeLang, setActiveLang] = useState('en');

  // Manual translations are used for better accuracy and premium feel
  useEffect(() => {
    const savedLang = localStorage.getItem('mhc_lang');
    if (savedLang) {
      setLang(savedLang);
  ...
  const speak = (text, langCode = activeLang) => {
    if (!window.speechSynthesis) return;
    window.speechSynthesis.cancel();
    const utterance = new SpeechSynthesisUtterance(text);
    
    // Auto-detect best voice (Prefer Google or Premium voices if available)
    const voices = window.speechSynthesis.getVoices();
    const preferredVoice = voices.find(v => v.lang.includes(langCode === 'ta' ? 'ta' : 'en') && (v.name.includes('Google') || v.name.includes('Premium')));
  ...
  const handleRegister = async (e) => {
    ...
    if (userName && contact) {
      ...
      localStorage.setItem('mhc_weight', userWeight);
      localStorage.setItem('mhc_height', userHeight);

      setLoadingText('Saving Health Profile...');
      setIsLoading(true);
    ...
    <div className={`min-h-[100dvh] font-sans ${offline ? 'bg-gray-100' : 'bg-gradient-to-br from-rose-50 to-teal-50'} flex justify-center text-gray-800`}>
      <div className="w-full max-w-md bg-white shadow-2xl relative overflow-hidden flex flex-col h-[100dvh]">
        ...
        <div className="flex-1 overflow-y-auto relative">
            ...
            <FullScreenOverlay onClose={() => navigateTo('dashboard', 'Going to Home...')} title={t.aiAssistant} bgColor="bg-slate-50">
              <div className="flex flex-col h-[calc(100vh-140px)]">
                <div className="flex-1 overflow-y-auto p-4 space-y-4">
                  <div className="bg-white p-4 rounded-2xl rounded-tl-none shadow-sm max-w-[85%] border border-indigo-50 relative group">
                    ...
                    <button 
                      onClick={() => speak(`வணக்கம் ${userName || 'தாயே'}. உங்கள் அறிகுறிகளைத் தேர்ந்தெடுக்கவும் அல்லது என்னிடம் நேரடியாகக் கேட்கவும். நான் உங்களுக்கு வீட்டு வைத்தியம் மற்றும் ஆரோக்கிய ஆலோசனைகளை வழங்குவேன்.`, 'ta')}
                      className="absolute -right-10 top-2 p-2 bg-indigo-50 text-indigo-500 rounded-full opacity-60 hover:opacity-100 transition-opacity"
                    >
                  ...
                  {chatMessages.map((msg, idx) => (
                    <div key={idx} className={msg.role === 'user' ? "ml-auto bg-indigo-500 text-white p-4 rounded-2xl rounded-tr-none shadow-sm max-w-[85%]" : "bg-white p-4 rounded-2xl rounded-tl-none shadow-sm max-w-[85%] border border-rose-100 relative overflow-hidden"}>
                      ...
                      {msg.role === 'ai' && (
                        <button 
                          onClick={() => speak(msg.text.replace(/<[^>]*>?/gm, ''), 'ta')}
                          className="mt-2 flex items-center space-x-1.5 text-[10px] font-black uppercase text-indigo-500 hover:text-indigo-700 transition-colors"
                        >
                          <Play size={12} className="fill-current" />
                          <span>Listen (கேட்கவும்)</span>
                        </button>
                      )}
                    </div>
                  ))}
          ...
          {screen === 'exercises' && <Exercises navigateTo={navigateTo} t={t} speak={speak} />}
          {screen === 'fruit-guide' && <FruitGuide navigateTo={navigateTo} t={t} speak={speak} />}
