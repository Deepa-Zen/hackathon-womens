...
import { Apple, AlertTriangle, CheckCircle2, Download, Info, ChevronDown, ChevronRight, Play } from 'lucide-react';
...
export default function FruitGuide({ navigateTo, t, speak }) {
    ...
    return (
        <FullScreenOverlay onClose={() => navigateTo('dashboard', 'Going to Home...')} title="Fruit Nutrition Guide" bgColor="bg-slate-50">
            <div className="p-4 space-y-6">
                ...
                {/* Warning Card */}
                <div className="bg-rose-50 border-l-4 border-rose-500 p-5 rounded-2xl shadow-sm flex items-start justify-between">
                    <div className="flex-1">
                        ...
                        <p className="text-xs font-bold text-slate-700 leading-relaxed">
                            Doctor அவர்களிடம் கலந்தாலோசித்த பிறகு மட்டுமே பின்பற்றவும்! <br/>
                            <span className="text-slate-400 font-medium">பழங்களை நன்கு கழுவி, பிரஷ்ஷாக (Fresh) சாப்பிடவும்.</span>
                        </p>
                    </div>
                    <button 
                        onClick={() => speak("முக்கிய எச்சரிக்கை. மருத்துவர் அறிவுரைப்படி பழங்களை சாப்பிடவும். பழங்களை நன்கு கழுவி பிரஷ்ஷாக சாப்பிடவும்.", "ta")}
                        className="bg-rose-100 p-3 rounded-full text-rose-600 animate-pulse active:scale-95 transition-transform"
                    >
                        ...
                        <div className="grid grid-cols-1 gap-4">
                            ...
                            <div className="bg-white rounded-[32px] p-6 shadow-sm border border-slate-100 border-t-4 border-t-emerald-500">
                                <div className="flex items-center justify-between mb-5">
                                    ...
                                    <button 
                                        onClick={() => speak(`இந்த மாதம் நீங்கள் சாப்பிட வேண்டிய பழங்கள்: ${data.safe.join(", ")}. இதன் நன்மைகள்: ${data.benefits.join(". ")}`, "ta")}
                                        className="bg-emerald-100 p-2 rounded-full text-emerald-600 active:scale-90 transition-transform"
                                    >
                            ...
                            <div className="bg-white rounded-[32px] p-6 shadow-sm border border-slate-100 border-t-4 border-t-rose-500">
                                <div className="flex items-center justify-between mb-5">
                                    ...
                                    <button 
                                        onClick={() => speak(`தவிர்க்க வேண்டிய பழங்கள்: ${data.avoid.join(", ")}. காரணம்: ${data.reasons.join(". ")}`, "ta")}
                                        className="bg-rose-100 p-2 rounded-full text-rose-600 active:scale-90 transition-transform"
                                    >
                                        <Play size={16} className="fill-current" />
                                    </button>
                                </div>
