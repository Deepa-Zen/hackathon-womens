...
import { ChevronRight, ChevronLeft, CheckCircle2, Play, Pause, RotateCcw, AlertTriangle, Star, Clock, Flame, Heart, Trophy, X, Volume2 } from 'lucide-react';
...
// ── Animated Pose Card ──────────────────────────────────────────────────────
function PoseCard({ pose, isCompleted, onComplete, speak }) {
        ...
        <div className="flex items-center space-x-3">
          ...
          <div>
            <div className="flex items-center space-x-2">
              ...
              <button 
                onClick={() => speak(`${pose.nameTa}. இந்த பயிற்சியின் நன்மைகள்: ${pose.benefit}. இப்போது செய்யும் முறையை கவனியுங்கள்: ${pose.steps.join('. ')}`, 'ta')}
                className="w-10 h-10 bg-indigo-600 text-white rounded-full flex items-center justify-center hover:bg-indigo-700 transition-all shadow-lg active:scale-90 hover:scale-110 group relative"
                title="Voice Guide"
              >
                <div className="absolute inset-0 bg-indigo-400 rounded-full animate-ping opacity-20 pointer-events-none"></div>
                <Volume2 size={20} />
                <span className="absolute -top-8 left-1/2 -translate-x-1/2 bg-slate-800 text-white text-[8px] font-black py-1 px-2 rounded opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap uppercase tracking-widest">குரல் வழி</span>
              </button>
            </div>
...
// ── Main Exercises Page ─────────────────────────────────────────────────────
export default function Exercises({ navigateTo, t, speak }) {
